...
class Messaging(MessagingABC):
    def __init__(
        ...
        self.client: aiopulsar.Client = aiopulsar.Client(service_url)
        self.lock: asyncio.Lock = asyncio.Lock(loop=self.loop)
        self.consumers: Dict[Tuple[str, str], aiopulsar.Consumer] = dict()
        self.producers: Dict[str, aiopulsar.Producer] = dict()

    async def cleanup(self) -> None:
        await self.client.close()

    def _get_consumer(self, topic_name: str, group_name: str) -> aiopulsar.Consumer:
        key = topic_name, group_name
        return self.consumers[key]

    async def _get_producer(self, topic_name: str) -> aiopulsar.Producer:
